...
Allowed DNS requests may be forwarded to the configured upstream DNS resolver so normal internet browsing can work. DNS resolvers can see DNS queries they receive, subject to their own privacy policies.

## Permissions

The app may request the following Android permissions:

- VPN permission: required to create the local DNS filtering VPN.
- Notification permission: used to show the foreground service status notification on supported Android versions.
- Internet permission: required to forward allowed DNS queries to upstream DNS resolvers.
- Foreground service permission: required to keep protection running while the app filters DNS traffic.
...
Users can stop protection at any time using the app's Stop button, the notification action, or Android VPN settings. Users can uninstall the app to remove all local app data.
